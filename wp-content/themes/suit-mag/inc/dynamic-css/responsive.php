 ...
 */
function suit_mag_responsive_device_css(){
	foreach( array( 'md' => 'desktop', 'sm' => 'tablet', 'xs' => 'mobile' ) as $size => $device ){

		Suit_Mag_Css::add_styles( array(
			#Typography
			array(
			...
			array(
				'selector'  => Suit_Mag_Helper::with_prefix_selector( '%s-post .post-title a' ),
				'props'		=> array(
			...
			array(
				'selector' => Suit_Mag_Helper::with_prefix_selector( '%s-inner-banner-wrapper' ),
				'props' => array(
...
add_action( 'init', 'suit_mag_responsive_device_css' );
