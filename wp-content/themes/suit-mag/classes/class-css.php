 ...
 */
if( !class_exists( 'Suit_Mag_Css ' ) ):
	class Suit_Mag_Css extends Suit_Mag_Helper{
					...
					<?php
						foreach( $css[ 'props' ] as $prop => $setting ){
							...
							if( is_array( $setting ) ){
								...
								if( $customizer ){
									$value = suit_mag_get( $setting[ 'value' ] );
								}else{
									$value = $setting[ 'value' ] ? $setting[ 'value' ] : '';
								}
								$unit  = $setting[ 'unit' ];
							}else{
								$value = suit_mag_get( $setting );
								if( 'font-family' == $prop ){
		...
		*/
		public static function print_styles(){
			...
			<style type="text/css" media="all" id="<?php echo esc_attr( self::with_prefix( 'styles' ) ) ?>">
				...
				<?php 
				$featured_gradiant_color_one = suit_mag_get( 'featured-gradiant-color-one' );
				$featured_gradiant_color_two = suit_mag_get( 'featured-gradiant-color-two' );
				$gradiant = ".suitmag-feature-news-wrapper article:after{
