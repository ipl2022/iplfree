...
if( !class_exists( 'Suit_Mag_Customizer' ) ){
	class Suit_Mag_Customizer extends Suit_Mag_Helper{
		...
		*/
		public static function add_custom_control( $control ){
			...
			if( !is_array( $control ) ){
				$err = esc_html__( 'Invalid argument passed for Custom Control. Must be an array with "type" & "class" key.', 'suit-mag' );
				self::add_error( $err );
				return;
			}
			...
			if( !isset( $control[ 'type' ] ) ){
				$err = esc_html__( 'Custom Control must contain "type" key ( type of control ).', 'suit-mag' );
				self::add_error( $err );
				$flag = false;
			}

			if( !isset( $control[ 'class' ] ) ){
				$err = esc_html__( 'Custom Control must contain "class" key ( name of custom class ).', 'suit-mag' );
				self::add_error( $err );
				$flag = false;
			}		

			if( !isset( $control[ 'sanitize' ] ) ){
				$err = esc_html__( 'Custom Control must contain "sanitize" key ( a callback function ).', 'suit-mag' );
				self::add_error( $err );
				$flag = false;
			}
		...
		*/
		public static function add_custom_section( $section ){
			...
			if( !is_array( $section ) ){
				$err = esc_html__( 'Invalid argument passed for Custom Section. Must be an array with "type" & "class" key.', 'suit-mag' );
				self::add_error( $err );
				return;
			}
			...
			if( !isset( $section[ 'type' ] ) ){
				$err = esc_html__( 'Custom Section must contain "type" key ( type of section ).', 'suit-mag' );
				self::add_error( $err );
				$flag = false;
			}

			if( !isset( $section[ 'class' ] ) ){
				$err = esc_html__( 'Custom Section must contain "class" key ( name of section class ).', 'suit-mag' );
				self::add_error( $err );
				$flag = false;
			}		
				...
				foreach( $args[ 'fields' ] as $field ){
					...
					if( isset( $field[ 'id' ] ) ){
						...
						if( array_key_exists( $field_id, self::$settings ) ){
							...
							$msg = sprintf( esc_html__( 'Field with ID: %s already exists.', 'suit-mag' ), $field[ 'id' ] );
							self::add_error( $msg );
						}
		...
		*/
		private static function validate_argument( $args ){
			...
			if( !is_array( $args ) ){
				...
				$msg = esc_html__( 'You must pass array as an argument in set method.', 'suit-mag' );
			...
			if( !isset( $args[ 'section' ] ) ){
				...
				$msg = esc_html__( 'No section found.', 'suit-mag' );
				self::add_error( $msg, 2 );
			...
			if( !is_array( $args[ 'section' ] ) ){
				...
				$msg = sprintf( 
					...
					esc_html__( 'Invalid argument', 'suit-mag' ),	
					esc_html__( 'section', 'suit-mag' ),
					esc_html__( 'Must be a an array with atleast an id, title...', 'suit-mag' )
				);
				...
				$msg = esc_html__( 'Section id is missing.', 'suit-mag' );
				self::add_error( $msg, 2 );
			}
			...
			if( isset( $args[ 'fields' ] ) && !is_array( $args[ 'fields' ] ) ){
				...
				$msg = sprintf( 
					'%s <strong><em>%s</em></strong>. %s',
					esc_html__( 'Invalid argument', 'suit-mag' ),	
					esc_html__( 'fields', 'suit-mag' ),
					esc_html__( 'Must be a an array', 'suit-mag' )
				);
						...
						<p>
							<?php  
								echo sprintf( '<strong>%s</strong> %s', 
									esc_html__( 'Suitmag Customizer:', 'suit-mag' ),
									esc_html( $value ) 
								);  
							?>
						</p>
		...
		*/
		private static function get_panel_arg( $panel ){
			...
			$args = array(
			    ...
			    esc_html__( 'No Title Specified.', 'suit-mag' ) : 
			    $panel['title'],
			);
		...
		*/
		private static function get_section_arg( $panel_id = false, $section ){
			$args = array(
				'title' => empty( $section[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'suit-mag' ) : $section[ 'title' ],
			);
...
add_action( 'customize_register', array( Suit_Mag_Customizer::get_instance(), 'register' ) );

if( !function_exists( 'suit_mag_get' ) ):
	/**
	 ...
	 * @package Suit Mag WordPress Theme
	 */
	function suit_mag_get( $id ){
		$id = Suit_Mag_Helper::with_prefix( $id );
		return Suit_Mag_Customizer::get( $id );
	}
