 ...
 */
if( !class_exists( 'Suit_Mag_Helper' ) ):
	class Suit_Mag_Helper{
		...
		public static function skip_content(){
			...
			<a class="skip-link screen-reader-text" href="#content">
				<?php esc_html_e( 'Skip to content', 'suit-mag' ); ?>
			</a>
			<?php
		}
		...
		public static function with_prefix_selector( $content ){
			return esc_html( str_replace( '%s', '.' . self::get_prefix(), $content ) );
		}
		...
		public static function excerpt_more( $text = false ){
			...
			return esc_html( '%' . self::get_prefix() . '-readmore%' );
		}
		...
		*/
		...
		public static function get_comment_number(){
			if( !suit_mag_get( 'post-comments' ) ){
				return;
			}
			
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				echo '<span class="suitmag-comments">';
				comments_popup_link(
					'<i class="fa fa-comment"></i> '.esc_html__( 'Leave a comment', 'suit-mag' ),		
					'<i class="fa fa-comment"></i> '.esc_html__( '1 response', 'suit-mag' ),
					'<i class="fa fa-comments"></i> % '. esc_html__( 'responses' , 'suit-mag' )
				);
				echo '</span>';
			}
		}		
		...
		*/
		public static function setup(){
			...
			add_theme_support( 'custom-header', apply_filters( self::fn_prefix( 'custom_header_args' ) , array(
				...
				'default-image'      => esc_url( get_template_directory_uri() . '/assets/img/default-banner.jpg' ),
			)));
		}
		...
		*/
		public static function get_assets_version(){
			return esc_html( suit_mag_get( 'assets-version' ) );
		}
		    ...
		    switch( $location ){
		        ...
		        case 'primary':
		            ...
		            $menu = wp_nav_menu( array(
		                ...
		                'container_class' =>'suitmag-main-menu'
		            ));
		        break;
		...
		*/
		public static function posts_navigation( $echo = true ) {
			...
			if( !$infinity_module_active || !Suit_Mag_Helper::is_active_plugin( 'jetpack' ) ){				
				the_posts_pagination(
					array(
						'mid_size'  => 2,
						'prev_text' => esc_html__( 'Previous', 'suit-mag' ),
						'next_text' => esc_html__( 'Next', 'suit-mag' ),
					)
				);
			}
		}
		...
		*/
		public static function post_content_navigation(){
			wp_link_pages( array(
				'before'      => '<div class="page-links">' . esc_html__( 'Pages:', 'suit-mag' ),
                'after'       => '</div>',
		...
		*/
		public static function single_post_navigation(){
			the_post_navigation( array(
				'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous Post', 'suit-mag' ) . '</span><span class="nav-title">%title</span>',
				'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next Post', 'suit-mag' ) . '</span><span class="nav-title">%title</span>',
			));
		}
		...
		*/
		...
		public static function the_date( $post_id = null, $status = 'posted' ) {
			...
			if( $show ):
				...
				printf(
					...
					( 'posted' == $status ) ? esc_html__( 'On', 'suit-mag' ) : esc_html__( 'Updated on', 'suit-mag' ),
					$time_tag
				);
			endif;
		}
		...
		public static function the_breadcrumb(){
			...
			if( apply_filters( self::fn_prefix( 'show_breadcrumb' ), true ) ): 
			    ...
			    <div id="<?php echo esc_attr( self::with_prefix( 'breadcrumb' ) ) ?>" class="<?php echo esc_attr( $classes ); ?>">
			    	<?php suitmag_breadcrumb_trail( $breadcrumb_args ); ?>
				</div><!-- #breadcrumb -->
			    <?php return;
			endif;
		}
					...
					<?php 
						...
						<?php }elseif(  is_search() ){
							...
							<h2 class="entry-title">
								<?php 
									printf( 
										esc_html__( 'Search Results for: %s', 'suit-mag' ), 
		...
		/**
		* Font family used in Suitmag Theme
		*
		* @static
		* @access public
		* @return object
		...
		*/
		public static function get_font_family( ){
			return apply_filters( self::fn_prefix( 'standard_fonts_array' ), array(
				'font-1'  => esc_html__( 'Lato', 'suit-mag' ),
				'font-2'  => esc_html__( 'Oswald', 'suit-mag' ),
				'font-3'  => esc_html__( 'Montserrat', 'suit-mag' ),
				'font-4'  => esc_html__( 'Roboto', 'suit-mag' ),  
				'font-5'  => esc_html__( 'Raleway', 'suit-mag' ),  
				'font-6'  => esc_html__( 'Playfair Display', 'suit-mag' ),  
				'font-7'  => esc_html__( 'Fjalla One', 'suit-mag' ),  
				'font-8'  => esc_html__( 'Alegreya Sans', 'suit-mag' ),
				'font-9'  => esc_html__( 'PT Sans Narrow', 'suit-mag' ),
				'font-10' => esc_html__( 'Open Sans', 'suit-mag' ),
				'font-11' => esc_html__( 'Poppins', 'suit-mag' ),
				'font-12' => esc_html__( 'Muli', 'suit-mag' ),
			));
		}
		...
		public static function get_google_font(){
			...
			$google_fonts[] = self::uglify( self::get_font( suit_mag_get( 'body-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( suit_mag_get( 'heading-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( suit_mag_get( 'site-info-font' ) ), $condition );
