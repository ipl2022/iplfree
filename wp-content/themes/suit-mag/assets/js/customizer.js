...
!function (t, n) {
  n.sectionConstructor["suit-mag-anchor"] = n.Section.extend({
...
wp.customize.controlConstructor['suit-mag-buttonset'] = wp.customize.Control.extend({
  ...
  jQuery(document).ready(function ($) {
    ...
    $('.alpha-color-control').each(function () {
      ...
      colorPickerOptions = {
        ...
        palettes: suitmagColorPalette.colorPalettes // Use the passed in palette.
      ...
      $('<div class="alpha-suit-mag-color-picker-container">' + '<div class="min-click-zone click-zone"></div>' + '<div class="max-click-zone click-zone"></div>' + '<div class="alpha-slider"></div>' + '<div class="transparency"></div>' + '</div>').appendTo($container.find('.wp-picker-holder'));
...
wp.customize.controlConstructor['suit-mag-dimensions'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
jQuery(document).ready(function ($) {
  ...
  $('.linked').on('click', function () {
    ...
    $this.parent('.link-suit-mag-dimensions').removeClass('unlinked');
  }); // Unlinked button
  ...
  $('.unlinked').on('click', function () {
    ...
    $this.parent('.link-suit-mag-dimensions').addClass('unlinked');
...
wp.customize.controlConstructor['suit-mag-icon'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
jQuery(window).load(function () {
  ...
  jQuery('.customize-control-suit-mag-radio-image .buttonset').buttonset(); // Handles setting the new value in the customizer.

  jQuery('.customize-control-suit-mag-radio-image input:radio').change(function () {
...
wp.customize.controlConstructor['suit-mag-range'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
    ...
    var control = this,
        ...
        input_default,
        changeAction,
        suitmag_range_input_number_timeout; // Update the text value
    ...
    function autocorrect_range_input_number(input_number, timeout) {
      ...
      clearTimeout(suitmag_range_input_number_timeout);
      suitmag_range_input_number_timeout = setTimeout(function () {
        if (isNaN(value)) {
...
wp.customize.controlConstructor['suit-mag-slider'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
(function ($, api) {
  api.controlConstructor['suit-mag-toggle'] = api.Control.extend({
    ready: function ready() {
      var control = this;
